    ...
    <>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {rows.map((r, i) => (
          <div
            key={i}
            className={`gmap-row ${activeGesture === r.id ? 'active' : ''}`}
            onMouseEnter={() => onSimulate?.(r.id)}
            onMouseLeave={() => onSimulate?.(null)}
          >
            <div className="gmap-glyph">{r.glyph}</div>
            <div className="gmap-info">
              <div className="gmap-trigger">{r.trigger}</div>
              <div className="gmap-target">
                <span style={{ opacity: 0.55 }}>{r.sub}</span>
              </div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
              <span className="gmap-arrow">→</span>
              <span className="gmap-chip">{r.target[0]} {r.target[1]}</span>
            </div>
          </div>
        ))}
      </div>
